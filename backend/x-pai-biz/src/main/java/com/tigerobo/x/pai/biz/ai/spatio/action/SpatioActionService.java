...
import com.tigerobo.x.pai.biz.ai.convert.SpatioActionConvert;
...
@Slf4j
@Service
public class SpatioActionService implements IBusinessDetailFetchService {
    ...
    BusinessEnum businessEnum = SpatioConstant.businessEnum;

    @Autowired
    private HotBusinessService hotBusinessService;


    public PageVo<AiSpatioActionVo> getPublishList(ArtImagePublicPageReq req) {
    ...
    public PageVo<AiSpatioActionVo> getNewPublishList(ArtImagePublicPageReq req) {

        Integer userId = ThreadLocalHolder.getUserId();
        Page<AiSpatioActionPo> userPage = aiSpatioActionDao.getPublishListQuery(req);

        PageVo<AiSpatioActionVo> pageVo = new PageVo<>();

        pageVo.setTotal(userPage.getTotal());
        pageVo.setPageSize(req.getPageSize());
        pageVo.setPageNum(req.getPageNum());

        List<AiSpatioActionVo> vos = SpatioActionConvert.po2vos(userPage);
    ...
    public PageVo<AiSpatioActionVo> getMyList(ArtImagePublicPageReq req) {
        Integer userId = ThreadLocalHolder.getUserId();
        Page<AiSpatioActionPo> userPage = aiSpatioActionDao.getUserPage(userId, req);
    ...
    public PageVo<AiSpatioActionVo> getHotList(ArtImagePublicPageReq req) {
        Integer userId = ThreadLocalHolder.getUserId();
        ...
        PageVo<AiSpatioActionVo> hotPage = new PageVo<>();
        ...
        List<Integer> effectIds = aiSpatioActionDao.getEffectByIds(ids, req.getKeyword());
        ...
        List<AiSpatioActionPo> pos = aiSpatioActionDao.getByIds(pageIds);
        hotPage.setTotal(hotPage.getTotal());
        hotPage.setPageSize(req.getPageSize());
        hotPage.setPageNum(req.getPageNum());

        List<AiSpatioActionVo> vos = SpatioActionConvert.po2vos(pos);

        List<AiSpatioActionVo> sortVos = SortUtil.sortByIdIndex(pageIds, vos);
    ...
    private void initOwner(List<AiSpatioActionVo> vos, Integer loginUserId) {

        if (CollectionUtils.isEmpty(vos)) {
            return;
        }

        for (AiSpatioActionVo vo : vos) {
    ...
    private void buildListUsers(List<AiSpatioActionVo> vos, boolean needFollowProperty) {
        if (CollectionUtils.isEmpty(vos)) {
            return;
        }
        ...
        Map<String, InteractVo> interactVos = interactionService.completeCount(bizIds, SpatioConstant.businessEnum.getType());
    ...
    public AiSpatioActionVo getByReqId(String userUuid, Long reqId) {
        Validate.isTrue(reqId != null, "参数不正确");
        Integer userId = userService.getIdByUuidCache(userUuid);
        Validate.isTrue(userId != null && userId > 0);
        AiSpatioActionPo imagePo = aiSpatioActionDao.getByReqId(reqId);
    ...
    public AiSpatioActionVo getByReqId(Long reqId) {
        Validate.isTrue(reqId != null, "参数不正确");
        AiSpatioActionPo imagePo = aiSpatioActionDao.getByReqId(reqId);

        return buildDetailVo(imagePo);
    }
    public AiSpatioActionVo getDetail(Integer id) {
        IdReqVo idReqVo = new IdReqVo();
        idReqVo.setId(id);
        return getDetail(idReqVo);
    }
    public AiSpatioActionVo getDetail(IdReqVo req) {

        AiSpatioActionVo vo = getById(req.getId());
    ...
    public AiSpatioActionVo getById(Integer id) {
        Validate.isTrue(id != null, "参数不正确");
        AiSpatioActionPo imagePo = aiSpatioActionDao.load(id);
        return buildDetailVo(imagePo);
    }

    private AiSpatioActionVo buildDetailVo(AiSpatioActionPo imagePo) {
        ...
        AiSpatioActionVo aiArtImageVo = SpatioActionConvert.po2vo(imagePo);
