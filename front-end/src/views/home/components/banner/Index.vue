<template>
  <div class="relative creation flex">
    <div
      ...
      style="z-index: 10"
    >
      <div v-show="activeIndex == index" class="w-full h-full">
        <div
    ...
    >
      ...
      <div class="flex items-center justify-between input-box mb-2">
        <div class="text-gray-200 text-sm text-shadow">
          输入一句话，AI为你生成创意画作
        </div>
        <div
          class="text-sm text-gray-300 underline text-shadow cursor-pointer"
          @click="randomTips"
        >
          随机
        </div>
      </div>
      <div class="input-box relative" style="--tw-bg-opacity: 0.3">
        <input
          ref="inputRef"
          rows="1"
          :placeholder="`例：${tipData.text}`"
          class="light-textarea-box block border-none bg-white pr-6 pl-2"
          v-model="text"
          style="--tw-bg-opacity: 0"
          autofocus
          @keypress.enter="goCreate"
        />
      </div>
      <!-- 风格选择 -->
      <div
      ...
      >
        <div
          class="relative border border-2 h-24 rounded-lg truncate cursor-pointer border"
          :class="{
            'border-indigo-300': activeModify == v.name,
          }"
          v-for="(v, index) in modifierList"
          :key="index"
        ...
        >
          ...
          />
          <div
            class="relative text-xs bg-black w-full h-full text-white w-full py-1 truncate px-1"
            style="--tw-bg-opacity: 0.2"
          >
            <div class="absolute left-1 bottom-1 font-medium">
              {{ v.name }}
            </div>
          </div>
        </div>
      </div>
      <div style="width: 80%; max-width: 500px" class="mt-4">
        <el-button
          class="w-full"
          size="large"
          color="#ff99ab"
